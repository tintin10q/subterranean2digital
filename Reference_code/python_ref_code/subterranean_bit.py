...
def bits_to_hex_string(in_bits):
    ...
    for i in range(0, final_position, 8):
        temp_value = in_bits[i] + 2 * in_bits[i + 1] + 4 * in_bits[i + 2] + 8 * in_bits[i + 3] + 16 * in_bits[
            i + 4] + 32 * in_bits[i + 5] + 64 * in_bits[i + 6] + 128 * in_bits[i + 7]
...
def bits_to_bytearray(value):
    ...
    if (len(value) != 0):
        ...
        j = 0
        for i in range(0, final_position, 8):
            value_bytearray[j] = value[i] + 2 * value[i + 1] + 4 * value[i + 2] + 8 * value[i + 3] + 16 * value[
                i + 4] + 32 * value[i + 5] + 64 * value[i + 6] + 128 * value[i + 7]
            j += 1
        mult_factor = 1
        value_bytearray[j] = 0
...
def subterranean_round(state_bits):
    ...
    # print("Chi step: ", new_state_bits)
    # Iota step
    new_state_bits[0] ^= 1
    # print("Iota step: ", new_state_bits)
    # Theta step
    new_state_bits = [
...
def crypto_aead_encrypt(m: bytes, ad: bytes, nsec: bytes, npub: bytes, k: bytes, t_length_bytes: int):
    m_bits = bytearray_to_bits(m)
...
def crypto_aead_decrypt(c, ad, nsec, npub, k, t_length_bytes):
    ...
    npub_bits = bytearray_to_bits(npub)
    k_bits = bytearray_to_bits(k)
    m_bits, t_bits = subterranean_SAE_direct_decrypt(k_bits, npub_bits, ad_bits, c_bits, t_prime_bits,
                                                     t_length_bytes * 8)
    if (t_bits == []):
        return None
